...
quote and every bar carries the time it was retrieved.
...
**Input is checked against the column it lands in.** `symbol` is the only path parameter, and a
composed `@Ticker` constraint — declared once, applied at every endpoint, so a new endpoint cannot
be added with a weaker rule by accident — holds it to `varchar(16)`'s width and to the shape of a
ticker —
...
**The browser only ever makes same-origin requests, so there is no CORS configuration at all.**
The client calls the relative base `/api`, never an absolute host: Vite proxies it in development
and nginx proxies it in production. CORS cannot arise in either environment, so rather than carry
a configurable allow-list for a case neither deployment reaches — and an untested branch with it —
the application ships without one. A deployment that does serve the UI from a separate origin
would add it against a real requirement.
...
# Backend
cd Backend
./mvnw test          # 24 tests. No network, no Docker — starts its own PostgreSQL.
...
| `app.yahoo-finance.user-agent` | `Mozilla/5.0 (compatible; OaklandStockMarket/1.0)` | Sent on every upstream call |
...
| Method | Path | Notes |
|---|---|---|
| GET | `/api/stocks/default` | The monitored ticker, as `{"symbol":"AAPL"}` |
...
Three endpoints, all three used by the UI, all three answering JSON — so the client has one
request path rather than a JSON one and a text one. The overview carries the quote and the history
together, so a page load is one round trip and cannot show a quote and a history that straddle a
refresh; `POST /refresh` returns the same shape, so a successful refresh needs no follow-up
request.

`/default` stays separate from the overview rather than being folded into it, because the two
answer different questions and fail differently. Which ticker this deployment monitors is
configuration and is always knowable; whether anything has been stored for it is not. Merging them
would make a *successful* overview fetch the only way to learn the symbol — and on a cold database
that fetch is the `404` the empty state exists for, so the page would lose the symbol exactly when
it needs it to label itself and to build the refresh URL.
...
**What is implemented** — 24 backend tests across six classes and 27 frontend tests across nine
...
- **Web** (`StockControllerTest`, `StockApiIntegrationTest`) — the endpoint contracts, the two
  ...
  collaborator is touched, a lower-case symbol resolving to the same resource as its upper-case
  form, and money serialised as an exact decimal string at the scale the column stores.
...
showing the last data stored. Every quote and bar carries `retrievedAt`, so stale data is
...
overview response before either grows past a month of daily bars; authentication and rate limiting
...
- **REST API** — three endpoints, DTO responses, RFC 9457 error handling.
- **Tests** — 24 backend and 27 frontend, all passing without network or Docker, plus 1 opt-in
...
- **A quote's `previous_close` is a property of the interval it was retrieved at, but
  ...
  the provider.
- **`range` is not validated, and it is the parameter that controls cost.** `symbol` is held to
  the shape and width of its column and `interval` to the width of its own, but `range` is passed
  to Yahoo as given. `POST /refresh?range=max` is accepted and returns bars back to 1984 — a few
  thousand rows, upserted one statement each, and then carried in full by every overview response
  thereafter. Nothing here is unsafe, but a request that costs several hundred times what the
  default costs should be a deliberate choice rather than an unnoticed one. An allow-list of the
  ranges Yahoo actually supports would be a handful of lines, in the same place `interval` is
  already checked.
- The API has no authentication or rate limiting, including on `POST /refresh`, which calls a
  third party on demand.
- Yahoo's endpoints are unofficial and can change without notice. The one `live`-tagged test is
  the early warning; it is excluded from the default build on purpose.
- The overview returns every stored bar for the symbol. The table pages through it ten at a
  time, so the DOM stays small, but the client still fetches and holds the whole series — fine
  for a month of daily bars, not for years of them. The fix is a paged endpoint, not more
  client-side slicing.
...
3. A retention policy for observations, and pagination on the overview's history. The table
...
6. A range selector on the chart. This needs a windowed history endpoint; the earlier one was
   removed because nothing called it, and it is a small addition to the storage seam when a UI
   actually needs it.
